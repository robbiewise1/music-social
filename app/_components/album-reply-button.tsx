...
import {
  ...
  type AlbumReply,
} from "@/app/actions/album-replies";
...
export function AlbumReplyButton({
  ...
  async function handleReplySubmit(parentId: string) {
    ...
    const result = await addAlbumReply(albumPostId, replyText.trim(), parentId);
  ...
  async function handleToggleLike(commentId: string, currentlyLiked: boolean) {
    ...
    const result = await toggleAlbumCommentLike(commentId, currentlyLiked);
    if (result?.error) {
      setReplies((prev) =>
        prev
          ? prev.map((r) =>
  ...
  const repliesByParent = new Map<string, AlbumReply[]>();
